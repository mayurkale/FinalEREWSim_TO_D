...
/*	writeTimeToLog(3,IDcode,df_soc,df_fracsec); // Checkpoint time before sorting	*/
}
...
/*			pthread_mutex_unlock(&mutex_on_thread); // Added by KK on 19-Oct-2013*/
		...
		//	printf("Inside assign_df id = %d index %d front = %d,rear = %d\n",IDcode,index,front,rear);
			...
			unsigned int IDcode = to_intconvertor(df->idcode);		
			free(fsec);						
/*			writeTimeToLog(3,IDcode,df_soc,df_fracsec);*/
...
/* ----------------------------------------------------------------------------	*/
/* FUNCTION  intermediate_dispatch(int xx):                                	    */
/* ----------------------------------------------------------------------------	*/
...
/*	if(angleDiffApp == true && DDS == 3 && SPDCApp == true) {*/

/*		size = create_dataframe(index);*/
/*		printf("Combined data frame SIZE %d\n", size);*/

/*	} else if(angleDiffApp == true && DDS == 3 && LPDCApp == true) {*/

/*		// KK Design	*/
/*		/* 1. Check for local threshod violations. log if violated with timestamp*/
/*		   2.If violated then send dataframe_DDS2 and trigger peer iPDC. Also set the local threshold */
/*		violation flag  */
/*		   3. Store the data in database */
/*		size  = create_DDS2_dataframe(df);*/
/*		storeDataInDB(df);*/
/*	}*/
/*	else*/
/*		printf("Nakko = True angleDiffApp == %d && DDS == %d && LPDCApp == %d \n",angleDiffApp,DDS,LPDCApp);*/
...
/*		*/
/*		// KK Design*/
/*		// 1.Check for global threshold violation. If violated then create a model and resend the model parameters to LiPDCs by creating a frame+ log the violations*/
/*		angleDifference(df);*/
/*		writeTimeToLog(4,IDcode,tsb_soc,tsb_fracsec); // Checkpoint time after Apps have executed*/
/*	}*/
...
/*					*/
/*					if (sendto(temp_pdc->sockfd,dataframe, size, 0,	(struct sockaddr *)&temp_pdc->pdc_addr,sizeof(temp_pdc->pdc_addr)) == -1){ */
/*							perror("sendto");	}*/
/*				*/
...
/*					dataframe_DDS2[14] = 0x04;*/
/*					dataframe_DDS2[15] = 0x00;*/

/*				} else {*/

/*					dataframe_DDS2[14] = 0x00;*/
/*					dataframe_DDS2[15] = 0x00;*/
/*				}*/
/*				if(temp_pdc->port == UDPPORT) {*/

/*					if (sendto(temp_pdc->sockfd,dataframe_DDS2,size,0,(struct sockaddr *)&temp_pdc->pdc_addr,sizeof(temp_pdc->pdc_addr)) == -1)*/
/*							perror("sendto");					*/
/*				} else if((temp_pdc->port == TCPPORT) && (temp_pdc->tcpup == 1)) {*/

/*					if(send(temp_pdc->sockfd,dataframe_DDS2,size, 0)== -1) {*/
/*						perror("send");	*/
/*						temp_pdc->tcpup = 0;*/
/*						pthread_cancel(temp_pdc->thread_id);		*/
/*					}				*/
...
int create_dataframe(int index) {
	...
	while(temp_df != NULL) { // 1
		...
		sprintf(pmuInfoKey,"%d",to_intconvertor(temp_df->idcode));
        pmuKey = pmuInfoKey;
        entry.key = pmuKey;						
		...
		z = offset + ((struct PMUInfo *)found->data)->startIndex;
		((struct PMUInfo *)found->data)->visited = true;
	...
	memset(pmuInfoKey,'\0',50);
	sprintf(pmuInfoKey,"%d",to_intconvertor(cfgfirst->idcode));
    pmuKey = pmuInfoKey;
    entry.key = pmuKey;								
...
int create_cfgframe(struct cfg_frame *cfg) { 
...
void checkForMissingFrames(ENTRY *found) {

	ENTRY entry;
	ENTRY *found_item;

	if(((struct PMUInfo *)found->data)->visited == true) {
		
		((struct PMUInfo *)found->data)->visited = false;
		int nextid = ((struct PMUInfo *)found->data)->nextIdcode; 
		if( nextid != -1) {

			memset(pmuInfoKey,'\0',50);
			sprintf(pmuInfoKey,"%d",nextid);
        	pmuKey = pmuInfoKey;
        	entry.key = pmuKey;									
			hsearch_r( entry,FIND, &found_item,&hashForPMUInfo);
			if(found_item == NULL) {				
	...
	} else if (((struct PMUInfo *)found->data)->visited == false) {
	
		unsigned char *temp = malloc(3*sizeof(unsigned char));
		memset(temp,'\0',3);
		int_to_ascii_convertor(((struct PMUInfo *)found->data)->Idcode,temp);

		struct data_frame *curr_df;			
		curr_df = generate_dummy_dataframe(temp);
		free(temp);			

		int z = ((struct PMUInfo *)found->data)->startIndex;
